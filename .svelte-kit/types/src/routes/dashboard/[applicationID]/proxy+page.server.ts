...
export const actions = {
  acceptApplication: async ({ params, cookies }: import('./$types').RequestEvent) => {
    try {
      if (connectionStatus.status != ConnectionStates.connected) {
        await connectToDB();
      }
  ...
  rejectApplication: async ({ params, cookies }: import('./$types').RequestEvent) => {
    try {
      if (connectionStatus.status != ConnectionStates.connected) {
        await connectToDB();
      }
  ...
  deleteApplication: async ({ params }: import('./$types').RequestEvent) => {
