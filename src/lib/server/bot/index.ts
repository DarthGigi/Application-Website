...
import type { DiscordUser, User } from '../types/database';
import type { APIDMChannel } from 'discord-api-types/v10';
import type { Application } from '$lib/types/application';

export const DiscordBotBase = DiscordAPIBase + 'v10/';

function makeDiscordAPIRequest(endpoint: string, method: 'GET' | 'POST' | 'PUT' | 'DELETE', body?: unknown, headers?: Record<string, unknown>) {
  ...
  console.log(DiscordBotBase + endpoint, options);
...
export async function sendAcceptLog(user: DiscordUser, application: Application) {
  // dm user
  const dm: APIDMChannel = await (await makeDiscordAPIRequest('/users/@me/channels', 'POST', { recipient_id: user.User.id })).json();
  await makeDiscordAPIRequest(`channels/${dm.id}/messages`, 'POST', {
    embeds: [
      {
  ...
  await makeDiscordAPIRequest(`channels/${PUBLIC_SIRIUS_APPLICATION_ACCEPTED_LOG_ID}/messages`, 'POST', {
    embeds: [
      {
        fields: [],
...
export async function sendDenyLog(user: DiscordUser) {
  const dm: APIDMChannel = await (await makeDiscordAPIRequest('/users/@me/channels', 'POST', { recipient_id: user.User.id })).json();
  return await makeDiscordAPIRequest(`channels/${dm.id}/messages`, 'POST', {
    embeds: [
      {
...
export async function getUserData(userID: string) {
  return await makeDiscordAPIRequest(`guilds/${PUBLIC_SIRIUS_GUILD_ID}/members/${userID}`, 'GET');
}
