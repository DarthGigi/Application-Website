...
export async function sendAcceptLog(user: DiscordUser, application: Application) {
    ...
    await makeDiscordAPIRequest(`channels/${dm.id}/messages`, "POST", {embeds: [{
        title: '<:Sirius:1056924373648429096>  Application Accepted',
        description: 'Your application has been accepted! Welcome to the team!',
        color: 0x2b2d31
      }]});
      await makeDiscordAPIRequest(`channels/${PUBLIC_SIRIUS_APPLICATION_ACCEPTED_LOG_ID}/messages`, "POST", {embeds: [{
        fields: [],
...
export async function sendDenyLog(user: DiscordUser) {
    const dm: APIDMChannel = await (await makeDiscordAPIRequest("/users/@me/channels", "POST", {recipient_id: user.User.id})).json();
    return await makeDiscordAPIRequest(`channels/${dm.id}/messages`, "POST", {embeds: [
        {
